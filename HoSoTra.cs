...
namespace BTL_Nhom1
{
    public partial class HoSoTra : Form
    {
        ...
        private void HoSoTra_Load(object sender, EventArgs e)
        {
            ...
            txtTienPhat.Text = "0";
            txtTongTien.Text = "0";
            Funtions.FillDataToCombo("select MaTheMuon from TheMuon ", cbMaTM, "MaTheMuon", "MaTheMuon");
            cbMaTM.SelectedIndex = -1;
            Funtions.FillDataToCombo("select MaThuThu from ThuThu", cbMaThuThu, "MaThuThu", "MaThuThu");
            cbMaThuThu.SelectedIndex = -1;
            Funtions.FillDataToCombo("select MaViPham from ViPham", cbViPham, "MaViPham", "MaViPham");
            cbViPham.SelectedIndex = -1;
            Funtions.FillDataToCombo("select MaHSM from HoSoMuon", cbMaHSM, "MaHSM", "MaHSM");
        ...
        private void Load_ttHD()
        {
            ...
            txtNgayMuon.Text = Funtions.ConvertDateTime(Funtions.GetFieldValues(str));
            str = "select MaTheMuon from HoSoMuon where MaHSM=N'" + txtMaHSM.Text + "'";
            cbMaTM.Text = Funtions.GetFieldValues(str);
            str = "select HoTen from TheMuon where MaTheMuon =N'" + cbMaTM.Text + "'";
            txtTenNguoiMuon.Text = Funtions.GetFieldValues(str);
            str = "select TamUng from HoSoMuon where MaHSM=N'" + txtMaHSM.Text + "'";
            txtTamUng.Text = Funtions.GetFieldValues(str);
        }
        ...
        private void btnTraSach_Click(object sender, EventArgs e)
        {
            ...
            ResetValues();
            txtMaHST.Text = Funtions.CreateKey("HDT");
            txtNgayTra.Text = Funtions.ConvertDateTime( DateTime.Now.ToShortDateString());
        ...
        private void dataHST_Click(object sender, EventArgs e)
        {
                ...
                cbMaTM.Text = Funtions.GetFieldValues("select MaTheMuon from HoSoMuon where MaHSM=N'" + txtMaHSM.Text + "'");
                txtTenNguoiMuon.Text = Funtions.GetFieldValues("select HoTen from TheMuon where MaTheMuon=N'" + cbMaTM.Text + "'");
                txtMaSach.Text = dataHST.CurrentRow.Cells["MaSach"].Value.ToString();
                txtTinhTrang.Text = Funtions.GetFieldValues("select TinhTrang from ChiTietHSM where MaSach=N'" + txtMaSach.Text + "' and MaHSM=N'" + txtMaHSM.Text + "'");
                txtNgayMuon.Text = dataHST.CurrentRow.Cells["NgayMuon"].Value.ToString();
                txtDonGiaThue.Text = Funtions.GetFieldValues("select DonGiaThue from Sach where MaSach=N'" + txtMaSach.Text + "'");
        ...
        {
            txtTienPhat.Text = Funtions.GetFieldValues("select TienPhat from ViPham where MaViPham=N'" + cbViPham.SelectedValue + "'");
        }
        ...
        private void btnLuu_Click(object sender, EventArgs e)
        {
            ...
            if (!Funtions.CheckKey(sql)) // nếu mã hóa đơn không trùng hóa tiến hành lưu được nhiều mã
            {
                ...
                Funtions.Connect();
                sql = "INSERT INTO HoSoTra(MaHST, MaHSM, NgayTra, TongTien, SoTienThanhToan , MaThuThu) VALUES(N'" + txtMaHST.Text.Trim() + "',N'" + txtMaHSM.Text.Trim() + "' ,'" +Funtions.ConvertDateTime( txtNgayTra.Text.Trim()) + "',N'" + txtTongTien.Text.Trim() + "','" + txtSoTienThanhToan.Text.Trim()+"' , '"+ cbMaThuThu.SelectedValue + "')";
                //Funtions.RunSQL(sql);
            ...
            //Funtions.RunSQL(sql);
            SqlCommand myCommand1 = new SqlCommand(sql1, Funtions.con);                     // Khai báo đối tượng SqlCommand
            myCommand1.ExecuteNonQuery();
            string sql2 = "update Sach set SoLuong=SoLuong+1 where MaSach=N'" + txtMaSach.Text + "'";
            //Funtions.RunSQL(sql);
            SqlCommand myCommand2 = new SqlCommand(sql2, Funtions.con);                     // Khai báo đối tượng SqlCommand
            myCommand2.ExecuteNonQuery();
            string sql3 = "update TheMuon set SoLuongMuon = SoLuongMuon - 1 where MaTheMuon = N '" + cbMaTM + "'";
            SqlCommand myCommand3 = new SqlCommand(sql3, Funtions.con);                     // Khai báo đối tượng SqlCommand
            ...
            SqlCommand myCommand4 = new SqlCommand(sql4, Funtions.con);                     // Khai báo đối tượng SqlCommand
            myCommand4.ExecuteNonQuery();
            Load_Datagridview();
            // Cập nhật lại tổng tiền cho hóa đơn 
            tong = Convert.ToDouble(Funtions.GetFieldValues("SELECT TongTien FROM HoSoTra WHERE MaHST = N'" + txtMaHST.Text + "'and MaHSM= '" + txtMaHSM.Text + "'"));
            ...
            //Classes.Funtions.RunSQL(sql);
            SqlCommand myCommand5 = new SqlCommand(sql5, Funtions.con);                     // Khai báo đối tượng SqlCommand
